...
export async function extractFromVideo(filePath: string): Promise<RawExtractionResult | null> {
  const metadata = await getVideoMetadata(filePath);
  
  if (!metadata) {
    throw new MetadataNotFoundError(filePath, 'video', ['format.tags', 'streams.tags']);
  }
...
async function getVideoMetadata(filePath: string): Promise<VideoMetadata | null> {
  return new Promise((resolve, reject) => {
    ...
    process.on('close', (code) => {
      if (code !== 0) {
        reject(new ExternalCommandError('ffprobe', args, code || -1, stderr, filePath));
        return;
      }
    ...
    process.on('error', (error) => {
      reject(new ExternalCommandError('ffprobe', args, null, `Process error: ${(error as Error).message}`, filePath));
    });
  });
}