    ...
    for (const result of results) {
        ...
        if (result.workflow) {
            ...
            if (workflowData.prompts.length > 0) {
                ...
                const hasPositive = workflowData.prompts.some((p) => p.positive && p.positive !== '');
            ...
            if (workflowData.samplerSettings) {
                ...
                output += `\nSampler Settings:\n`;
                if (s.steps)
                    output += `  Steps: ${s.steps}\n`;
                if (s.cfg !== undefined)
                    output += `  CFG Scale: ${s.cfg}\n`;
                if (s.cfg_start !== undefined && s.cfg_end !== undefined) {
                    output += `  CFG Schedule: ${s.cfg_start} → ${s.cfg_end}\n`;
                }
                if (s.scheduler)
                    output += `  Scheduler: ${s.scheduler}\n`;
                if (s.seed !== undefined)
                    output += `  Seed: ${s.seed}\n`;
                if (s.denoise !== undefined)
                    output += `  Denoise: ${s.denoise}\n`;
            }
            // Model info
            ...
            // Basic stats
            const info = (0, validation_1.extractWorkflowInfo)(result.workflow);
            output += `\nWorkflow Stats:\n`;
...
function extractComfyUIWorkflowData(workflow) {
    const data = {
        loras: [],
        prompts: [],
    ...
    for (const [nodeId, node] of Object.entries(workflowNodes)) {
        ...
        if (classType === 'WanVideoLoraSelect' || classType?.includes('Lora') || classType === 'Power Lora Loader (rgthree)') {
            ...
            if (inputs.lora && inputs.strength !== undefined) {
                const loraName = inputs.lora.split('\\').pop()?.split('/').pop() || inputs.lora;
                data.loras.push({
                    name: loraName,
            ...
            if (inputs.text && typeof inputs.text === 'string') {
                ...
                if (loraMatches) {
                    loraMatches.forEach((match) => {
                        const [, loraPath, strengthStr] = match.match(/<lora:([^:>]+):([0-9.]+)>/) || [];
                        if (loraPath && strengthStr) {
                            const loraName = loraPath.split('\\').pop()?.split('/').pop() || loraPath;
 ...
 */
function findPromptConnections(workflowNodes) {
    const connections = {
        positive: [],
        negative: [],
    };
 ...
 */
function mergePromptPairs(prompts) {
    // Group prompts
    const positivePrompts = prompts.filter((p) => p.positive && p.positive !== '');
    ...
    if (positivePrompts.length > 0 && negativePrompts.length > 0) {
        for (let i = 0; i < Math.max(positivePrompts.length, negativePrompts.length); i++) {
